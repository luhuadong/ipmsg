  ...
  if (who>0)
  {
    ...
    while (1)
    {
     ...
     if (lstat(fileName, &fileAttr)<0)
      ...
      snprintf(tail->fileSize, sizeof(tail->fileSize), "%lx", fileAttr.st_size);
      snprintf(tail->mtime, sizeof(tail->mtime), "%lx", fileAttr.st_mtime);
...
int getFile(void* option, gsNode *gList)
{
  ...
  snprintf(com.additional, MSGLEN, "%x:%x:%lx", gList->packetNo, head->fileNo, offset);
  ...
  sscanf(head->fileSize, "%lx", &fileSize);
...
int getDir(void *option, gsNode *gList)
{
  ...
  do
  {
    ...
    case IPMSG_FILE_REGULAR:
      ...
      sscanf(fn.fileSize, "%lx", &fileSize);
      while (fileSize>0)
      {
        readBytes = fileSize < RECFRG ? fileSize:RECFRG;
        if ((readBytes = readn(sockfd, recvs, readBytes))<0) //RECFRG
